...
We use our algorithm to calculate an accurate benchmark energy for the chromium dimer with the relativistic X2C Hamiltonian in the cc-pVDZ-DK basis, correlating 28 electrons in 76 spatial orbitals.
Our largest calculation uses two billion Slater determinants in the variational space, and
semistochastically includes perturbative contributions from trillions of additional determinants with better than 0.01~mHa statistical uncertainty.
% semistochastically includes perturbative contributions from trillions of additional determinants to obtain better than 0.01~mHa statistical uncertainty.
...
Other SCI methods, such as CIPSI~\cite{HurMalRan-JCP-73,EvaDauMal-CP-83} use a different criterion 
\beq
\left|c_a^{(1)}\right|=\left|\frac{\sum_i H_{ai}c_i}{E_0-E_a}\right| > \epsilon_1
\label{eq:cipsi_ground}
\eeq
which is based on a first-order perturbative estimate of the expansion coefficient. The reason we choose instead the selection criterion in Eq.~\ref{HCI_criterion} is that it can be implemented
...
%Moreover, one could use the HCI criterion in Eq.~\ref{HCI_criterion} as a preselection criterion for selecting determinants
%using the criterion in Eq.~\ref{eq:cipsi_ground}.
...
The straightforward
and time-efficient approach to computing the perturbative correction requires storing and updating the
partial sum for each $a$ ($\frac{\left(\sum_{i \in \V} H_{ai} c_i\right)^2}{E_0 - E_a}$) by looping over all the determinants $i\in\V$. This creates a severe memory bottleneck.
...
%$i_D$, $i_\alpha$
$\vecD$ & The array of determinants in $\V$ \\
& in the order they were generated. \\
&\\
$\veca$ & The array of all alpha strings, without repeats that\\
&are present in at least one determinant in $\V$ in the \\
...
&\\
$\veciDa(\ia)$ & The array of determinant indices in $\vecD$ \\
& such that the alpha strings of those determinants \\
...
& The elements of $\veciDa(\ia)$ are sorted either by their \\
& values, or by the indices of their beta strings in $\vecb$. \\
& (See text for details.)\\
&\\
$\veciba(\ia)$ & The array of all beta string indices in $\beta$ \\
& that appear with $\alpha_{\ia}$ in a determinant. \\
& It is sorted so that the elements of $\veciDa(\ia)$ and $\veciba(\ia)$ \\
& are always in correspondence.\\
&\\
...
& less electron, $\alpha^{(-1)}$, and returns an array of indices \\
& of alpha strings in $\veca$ that can give $\alpha^{(-1)}$ \\
& upon removing an electron.  These are \\
...
\FOR{$D_i$ in $\vecD$}
  ...
  \State \hskip 4mm the indices of the $\alpha(D_i)$ and $\beta(D_i)$.
...
%       \IF {$\beta(D_j)$ present in $\vecb_{\beta_i}$ (binary search)}
        \IF {$\ib(\beta(D_j))\in\veciba(\ia)$ (binary search)}
          \State Compute and add $H_{ij}$ to the Hamiltonian
        \ENDIF
      \ENDFOR
    \ELSE
...
Note that if two $\alpha$ strings are a single excitation apart, they will be simultaneously present under one, and only one key of the hash map.
...
When the number of new determinants is less than 20\% of the total number of determinants (in the later iterations of a given $\epsilon_1$), $\veciDa(\ia)$ and $\veciba(\ia)$ are sorted by $i_D$, otherwise, they are sorted by $\ib$.
...
\begin{table}[h]
  ...
  \end{tabular}
    ...
    We also provide the timing to reach a 2~$\mu$Ha uncertainty to illustrate that our statistical error goes down much faster than $1/\sqrt{T}$ since we use smaller $\epsilon^{\rm dtm}_2$ and $\epsilon^{\rm psto}_2$ values for smaller target errors.
}
  \label{tab:pt}
\end{table}
...
It is always more efficient than the original algorithm, especially when run on computers with small memory, in which case the
...
reaonable choices for a target error around 0.01~mHa are  $\epsilon_2^{\rm dtm} = \max(\epsilon_{1}/50,10^{-6} {\rm Ha})$,
$\epsilon_2^{\rm psto} = \max(\epsilon_{1}/500,10^{-7} {\rm Ha})$, and,
...
Typically, we choose $\epsilon_2^{\rm dtm} = \max(\epsilon_{1}/50, 10^{-6} {\rm Ha})$ for a target error of $10^{-5}$~Ha.
...
Typically, for a target statistical error of $10^{-5}$ Ha, we choose $\epsilon_2^{\rm psto} = \max(\epsilon_{1}/500, 10^{-7} {\rm Ha})$.
...
number of variational determinants and all the the perturbative determinants that satisfy the $\epsilon_2$ criterion (the original SHCI algorithm),
or, by using larger number of variational determinants and selecting just one batch of the perturbative determinants.
...
Even when memory is abundant, the improved SHCI is still a few times times faster.
...
and,
(2) A small fraction of the batches in the pseudo stochastic step is usually sufficient to give an accurate estimation the total correction.
% the loop over batches in the batch-stochastic step is usually exited after processing only a small fraction of the batches.
...
In contrast, in the stochasitic step the fluctuations come both from the choice of variational determinants and the choice of batch.
...
It is an efficient deterministic algorithm for memory constrained environment, but for a reasonable statistical error tolerance, e.g., 0.01~mHa, a semistochastic approach is usually much faster, as we can see from Table~\ref{tab:pt}.
...
For transfering the determinants to other nodes or saving them to disk, we use base-128 variable-length integers
%(VarInts)~\cite{stuart2012encoding} to compress the integers.
(VarInts)~\cite{StuFer-Protobuf-12} to compress the 64-bit integers.
VarInts take only a few binary operations to compute and reduce the memory footprint by up to 87.5\% for small integers, which reduces the network traffic and the sizes of the wavefunction files considerably, especially for large basis sets.
...
During the matrix-vector multiplication, each node will apply its own trunk of the Hamiltonian to the given vector to get a partial resulting vector.
The partial results are then merged together by doing a binomial tree reduction.
The works on each node are distributed to the cores with dynamic load balancing.
...
To store this map from $D_a$ to $\sum_i H_{ai}c_{i}$, we use a distributed hash table.
...
The distributed hash table is based on lock-free open-addressing linear-probing concurrent hash tables specifically designed for commutative insertion and update operations at large scale, which is exactly the case for accumulating the partial sums.
...
Each concurrent hash table is implemented as lots of segments and each segment can be modified by only one thread at a time.
When a thread wants to insert or update a (key, value) pair, it will first check whether the segment that the key belongs to is being used by other threads.
...
For each $\alpha$ and $\beta$, we initialize a singles array and a lock to ensure exclusive modification.
%Then, we do the following for each $\alpha$ in parallel:
%We go through the $\alpha-1$ arrays of that $\alpha$ to find all the $\alpha^\prime$s that are one excitation away.
We loop over the $\alpha-1$ arrays of each $\alpha$ to find all the $\alpha^\prime$s that are one excitation away.
We lock and append $\alpha^\prime$ to the singles array of $\alpha$, and
we lock and append $\alpha$ to the singles array of $\alpha^\prime$.
When both $\alpha$ and $\alpha^\prime$ are in the new $\alpha$ array, only the smaller of the two does the update.
...
From 1 node to 4 nodes, we see significant deviation from linear speedup due to the additional communication from shuffling the perturbative determinants across nodes.
...
The largest variational calculation where we iteratively find and diagonalize 2 billion determinants for $\epsilon_1=3.0\times10^{-6}$ takes only one day on 8 nodes, each of which has 4 Intel Xeon E7-8870 v4 CPUs.
The corresponding perturbative calculation takes only 6 hours with one of these nodes. 
During that perturbative calculation, we skipped the deterministic step, and performed a pseudo stochastic step with $\epsilon_2^{\rm psto}=1\times10^{-7}$ Ha and a stochastic step with $\epsilon_2=3\times10^{-12}$Ha.
%why?
...
We run 10 iterations and obtain the additional correction from $\epsilon_2=3.0\times10^{-12}$ to be -0.001203(6)~Ha.
The combined uncertainty of the entire semistochastic perturbation stage is 6~$\mu$Ha.
It is hard to estimate how many determinants are stochastically included for $\epsilon_2=3\times10^{-12}$, so we estimate a lower bound with $\epsilon_2^{\rm psto}=1.4\times10^{-8}$ and obtain 1.8 trillion unique perturbative determinants.
Hence, with $\epsilon_2=3\times10^{-12}$, the value we are actually using, we stochastically estimate contributions from at least trillions of unique perturbative determinants and obtain better than 0.01~mHa statistical uncertainty in 6 hours using only one node.
%why?
...
%   \end{tabular}
  ...
  $|\V|$ is the number of variational determinants.
...
\bibliography{main,umrigar,toulouse,chan,hci}
%\bibliography{hci}
