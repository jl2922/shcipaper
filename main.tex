...
A another feature of the method is that if the calculation is done for various values of the variational threshold $\epsilon_1$
...
There are two straightforward ``brute force" approaches to build the Hamiltonian matrix:
...
% {\color{red} \lstinline{(n >> 6)} (MJO - is 6 correct? Or should it be 64? 6 seems odd}, where ``\lstinline{&}'' is the bitwise ``and'' and ``\lstinline{>>}'' is the bitwise right shift.
...
To see the contribution from each excitation level we plot the number of selected determinants and the $\sum_i \left|c_i\right|^2$ for each excitation level in Fig.~\ref{fig:excit}.
