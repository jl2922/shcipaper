...
% Table~\ref{auxiliary} and 
%the data structures and our new algorithms for constructing the Hamiltonian matrix.
% We discuss some additional details of the algorithm here.
%focus on some details that are absent there.
...
(n mod 64) can be implemented as \lstinline{(n & 63)}, and \lstinline{(n / 64)} can be implemented as {\color{red} \lstinline{(n >> 6)} (MJO - is 6 correct? Or should it be 64? 6 seems odd}, where ``\lstinline{&}'' is the bitwise ``and'' and ``\lstinline{>>}'' is the bitwise right shift.
...
Applications include providing benchmark energies for a variety of organic molecules, as well as for transition metal atoms, dimers, and monoxides, and providing calibration or training data for large scale methods,
such as interatomic potentials in molecular dynamics, exchange correlation functionals in density functional theory, and machine learning based quantum chemistry solvers.
