...
The choice of quantum chemistry method requires a trade-off between accuracy and efficiency.
...
%method~\cite{HurMalRan-JCP-73,BuePey-TCA-74,EvaDauMal-CP-83,Cim-JCP-85,CimPer-JCoP-87,Kno-CPL-89,Har-JCP-91,PovRubIll-TCA-92,SteWenWilWil-CPL-94,GarCasCabMal-CPL-95,WenSteWil-IJQC-96,Wul-CPL-96,Nee-JCP-03,NakEha-JCP-05,AbrShe-CPL-05,BytRue-CP-09,Rot-PRC-09,BenBesHoyMay-JCP-11,LiuHof-TCA-14,CoeMurPat-CPL-14,Eva-JCP-14,Kno-MP-15,SchEva-JCP-16,LiuHof-JCTC-16,ZhaEva-JCTC-16,SceAppGinCaf-JCoC-16,GarSceLooCaf-JCP-17,SchEva-JCTC-17,LooSceBloGarCafJac-JCP-18,TubLevHaiHeaWha-ARX-18},
...
evaluating this perturbation correction semistochastically~\cite{ShaHolJeaAlaUmr-JCTC-17}.
...
{\color{blue} For example, $N_s$ samples, each of size $N_d$, will have $N_s N_d^2$ contributions to the energy, whereas
...
so each batch has about the same number of determinants
, i.e., the fluctuations in the number of
determinants in the various batches is the square root of the average number of determinants in each batch and $\sigma_N\approx\sqrt{N}/N\to0$ for large $N$.
}
...
{\color{blue} all the variational determinants are used}, and
all the perturbative batches are summed over.
\item A ``pseudostochastic" step, with cutoff $\epsilon_2^{\rm psto} (< \epsilon_2^{\rm dtm})$, wherein
{\color{blue} all the variational determinants are used}, and
...
{\color{blue} Of course, if $\epsilon_1 \le \epsilon_2^{\rm dtm}$ the deterministic step is skipped.
}
...
{\color{blue} and a large number of batches},
...
and why we precede the stochastic step with the deterministic and pseudostochastic steps, but
...
{\color{blue} We have also performed some preliminary calculations for the energy surface with larger basis sets, such as cc-pwCVTZ. However, due to the lack of reference values and because the focus of this paper is on the accuracy and performance of the improved SHCI method, we leave the extensive study of the chromium dimer to future a future paper.}
...
\bibliography{main,hci,umrigar,toulouse,chan,needs,chemistry}
%\bibliography{hci}
